...
<body class="bg-light">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-4 shadow">
        <div class="container">
            <a class="navbar-brand font-weight-bold" href="<?= $_SERVER['SCRIPT_NAME'] ?>?acao=dashboard">Estética Automotiva</a>
            <div class="d-flex align-items-center text-white">
                <span class="me-3">Olá, <b><?= htmlspecialchars($_SESSION['usuario_nome'] ?? '') ?></b></span>
                <a href="<?= $_SERVER['SCRIPT_NAME'] ?>?acao=sair" class="btn btn-outline-danger btn-sm">Sair</a>
            </div>
        </div>
    </nav>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-5">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <form method="POST" action="<?= $_SERVER['SCRIPT_NAME'] ?>?acao=agendar">
                            <div class="mb-3">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($agendamentos as $item): ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($item['veiculo']) ?></td>
                                                <td><span class="badge bg-info text-dark"><?= htmlspecialchars($item['servico']) ?></span></td>
                                                <td><?= date('d/m/Y', strtotime($item['data_agendamento'])) . ' às ' . htmlspecialchars($item['horario']) ?></td>
                                                <td class="text-center">
                                                    <a href="<?= $_SERVER['SCRIPT_NAME'] ?>?acao=excluir&id=<?= $item['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Tem certeza que deseja excluir?');">Excluir</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
